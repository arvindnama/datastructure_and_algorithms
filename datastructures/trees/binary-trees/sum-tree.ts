...
function isSumTree(root: Nullable<TreeNode<number>>): boolean {
    const isSumTreeInt = (
    ...
    ): { sum: number; isSumTree: boolean } => {
        if (!root) return { sum: 0, isSumTree: true };
        if (!root.left && !root.right)
            return { sum: root.value as number, isSumTree: true };
        ...
        const isRootSumTree =
            sumOfChildren === root.value &&
            leftRes.isSumTree &&
            rightRes.isSumTree;

        return {
            sum: (sumOfChildren + root.value) as number,
            isSumTree: isRootSumTree,
        };
    };
