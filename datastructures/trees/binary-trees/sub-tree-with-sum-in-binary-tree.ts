...
function isSubTreeWithSumFound(r: TreeNode<number>, sum: number): boolean {
  ...
  const internalFn =  (r: Nullable<TreeNode<number>>): {sum: number, matchFound: boolean} => {
    if(!r) return {sum: 0, matchFound: sum === 0};
    if(!r.right && !r.left) return {sum: r.value, matchFound: sum === r.value}
    ...
    return {
      matchFound: (leftRes.matchFound || rightRes.matchFound) || (sumOfNode === sum),
      sum: sumOfNode
    }
  }
