...
function minStepsToReachTarget(
  ...
  const visited: boolean[][] = Array(n).fill([]).map(row => Array(n).fill(false));
  ...
  const canMove = ([x, y]: Coordinates) => x >=0 && y >=0 && x < n && y < n && !visited[x]?.[y];
  ...
  while(queue.length){
    const {c: [sx, sy], dist } = queue.shift() as {c: Coordinates, dist: number};
    if(sx === tx && sy === ty) {
      return dist;
    }
...
console.log('knight path from (1,3) to (5,0)', minStepsToReachTarget(6,[1,3],[5,0]));
console.log('knight path from (1,1) to (29,29)', minStepsToReachTarget(30,[1,1],[29,29]));