...
const solve0_1_KnapsackProblem = (
    ...
    const map = ((solve0_1_KnapsackProblem as any as FuncWithMap).map =
        (solve0_1_KnapsackProblem as any as FuncWithMap).map || {});

    if (w === 0 || n === 0) return 0; // return zero profit as we are at end.
    const key = `${n}_${w}`;
    if ((map[key] as number) >= 0) return map[key] as number;
    const profitIfItemPicked =
        w - weight[n - 1] < 0
            ? 0
            : profit[n - 1] +
    ...
    const profitIfItemNotPicked = solve0_1_KnapsackProblem(
        profit,
        weight,
        n - 1,
        w
    ...
    return (map[key] = Math.max(profitIfItemPicked, profitIfItemNotPicked));
};
